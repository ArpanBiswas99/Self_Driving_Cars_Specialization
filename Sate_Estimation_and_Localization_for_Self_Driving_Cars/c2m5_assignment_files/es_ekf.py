...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    q_check = Quaternion(axis_angle=imu_w.data[k - 1] * delta_t).quat_mult(q_est[k - 1])
    ...
    p_cov_check = F_k@p_cov_check@F_k.T + L_k@Q_km@L_k.T #p_cov

    # 3. Check availability of GNSS and LIDAR measurements
    K_k = p_cov_check@h_jac.T@inv(h_jac@p_cov_check@h_jac.T + np.eye(3)*lidar_data)
